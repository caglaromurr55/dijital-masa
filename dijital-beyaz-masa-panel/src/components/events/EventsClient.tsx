...
export default function EventsClient({ initialEvents, canEdit }: EventsClientProps) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm shadow-slate-200/50 dark:shadow-none">
                <div>
                    <h1 className="text-3xl font-black tracking-tight text-slate-900 dark:text-slate-100 uppercase">Etkinlik Yönetimi</h1>
                    <p className="text-slate-400 font-medium text-sm mt-1">Belediye etkinlik takvimini buradan profesyonelce yönetin.</p>
                </div>
                {canEdit ? (
                    <AddEventDialog onEventAdded={() => router.refresh()} />
                ) : (
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-orange-600 bg-orange-50 px-4 py-2 rounded-xl border border-orange-100">
                        <Lock size={14} /> Görüntüleme Modu
                    </div>
                )}
            </div>

            <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white/50 dark:bg-slate-900/50 backdrop-blur-md p-2 rounded-2xl border border-white dark:border-slate-800 shadow-sm">
                ...
                <div className="flex items-center gap-1 bg-slate-100/50 p-1.5 rounded-xl">
                    <Button
                        variant="ghost"
                        size="sm"
                        className={`rounded-lg h-9 px-4 transition-all ${viewMode === 'grid' ? 'bg-white dark:bg-slate-950 shadow-sm text-blue-600 dark:text-blue-400 font-bold' : 'text-slate-500 hover:bg-slate-200/50 dark:hover:bg-slate-800/50'}`}
                    ...
                    <Button
                        variant="ghost"
                        size="sm"
                        className={`rounded-lg h-9 px-4 transition-all ${viewMode === 'list' ? 'bg-white dark:bg-slate-950 shadow-sm text-blue-600 dark:text-blue-400 font-bold' : 'text-slate-500 hover:bg-slate-200/50 dark:hover:bg-slate-800/50'}`}
            ...
            {viewMode === "grid" ? (
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 pb-10">
                    {filteredEvents.map((event) => (
                        <Card
                            key={event.id}
                            className={`group transition-all rounded-[2.5rem] border-none shadow-sm flex flex-col ${event.is_active ? "bg-white dark:bg-slate-900 hover:shadow-2xl hover:shadow-blue-500/5 dark:hover:shadow-none" : "bg-slate-50 dark:bg-slate-950 opacity-75"
                                }`}
                        >
                            <CardHeader className="pb-4 relative px-8 pt-8">
                                ...
                                <CardTitle className={`mt-5 text-xl font-black leading-tight tracking-tight ${event.is_active ? "text-slate-900 dark:text-slate-100" : "text-slate-400 line-through"}`}>
                                    {event.title}
                                </CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-6 flex-1 flex flex-col justify-between px-8 pb-8">
                                ...
                                <div className="space-y-3 pt-6 border-t border-slate-50 mt-auto">
                                    ...
                                    <div className="flex items-center gap-3 p-3 rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-sm">
                                        ...
                                        <div className="flex flex-col leading-none">
                                            ...
                                            <span className="text-xs font-bold text-slate-700 dark:text-slate-200 truncate max-w-37.5">{event.location}</span>
                                        </div>
                                    </div>
                                </div>
                            </CardContent>
                ...
                <div className="rounded-[2.5rem] border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden mb-10">
                    <Table>
                        <TableHeader>
                                ...
                                <TableRow key={event.id} className="group hover:bg-blue-50/20 dark:hover:bg-blue-900/10 transition-colors border-b border-slate-50 dark:border-slate-800">
                                    <TableCell className="px-8 py-5">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <p className={`font-bold text-slate-900 dark:text-slate-100 ${!event.is_active && "text-slate-400 dark:text-slate-500 line-through"}`}>{event.title}</p>
