...
export default function Sidebar({ userProfile }: SidebarProps) {
  ...
  if (!userProfile) {
    if (isCollapsed) return <div className="w-20 bg-white border-r h-full" />;
    return (
      <div className="w-64 bg-white border-r h-full p-6 space-y-6">
  ...
  return (
    <div className={`relative flex flex-col h-full bg-white dark:bg-slate-950 border-r border-slate-100/50 dark:border-slate-800 transition-all duration-300 ease-in-out z-40 ${isCollapsed ? "w-20" : "w-64"}`}>
